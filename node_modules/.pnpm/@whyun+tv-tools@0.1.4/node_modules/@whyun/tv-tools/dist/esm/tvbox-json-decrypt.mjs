...
function rightPadding(value, fill, length) {
...
function decryptCbc(hex, key, iv) {
...
export function decodeTvboxJson(input, configKey) {
    let json = input;
    let content = input;
    try {
        if (isJson(content)) {
        ...
        if (content.startsWith('2423')) {
            const data = content.slice(content.indexOf('2324') + 4, content.length - 26);
            const decodedContent = hexToBuffer(content)
                .toString('utf8')
                .toLowerCase();
            const key = rightPadding(decodedContent.slice(decodedContent.indexOf('$#') + 2, decodedContent.indexOf('#$')), '0', 16);
            const iv = rightPadding(decodedContent.slice(decodedContent.length - 13), '0', 16);
...
export function parseTvboxJson(input, configKey) {
    return parseJsonValue(decodeTvboxJson(input, configKey));
}
//# sourceMappingURL=tvbox-json-decrypt.mjs.map